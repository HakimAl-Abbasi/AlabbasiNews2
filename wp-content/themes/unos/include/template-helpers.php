...
function unos_get_text_logo( $unos_logo, $tag_h1 = 'div', $tag_h2 = 'div' ) {
	...
	$class .= ( 'text' == $unos_logo && !function_exists( 'hoot_lib_premium_core' ) ) ? ' site-logo-text-' . hoot_get_mod( 'logo_size' ) : '';
	...
	$display .= '<div id="' . esc_attr( $id ) . '" class="' . esc_attr( $class ) . '">';
		...
		$display .= "<{$tag_h1} " . hoot_get_attr( 'site-title' ) . '>';
			$display .= '<a href="' . esc_url( home_url() ) . '" rel="home" itemprop="url">';
				...
				$title = ( 'custom' == $unos_logo ) ? unos_get_custom_text_logo() : '<span class="blogname">' . get_bloginfo( 'name' ) . '</span>';
		...
		if ( hoot_get_mod( 'show_tagline' ) && $desc = get_bloginfo( 'description' ) ) {
			...
			$noshow = ( hoot_get_mod( 'show_tagline' ) ) ? '' : 'noshow';
			$display .= "<{$tag_h2} " . hoot_get_attr( 'site-description', '', $noshow ) . '>' . get_bloginfo( 'description' ) . "</{$tag_h2}>";
		}
...
function unos_get_mixed_logo( $unos_logo, $tag_h1 = 'div', $tag_h2 = 'div' ) {
	...
	$class .= ( 'mixed' == $unos_logo && !function_exists( 'hoot_lib_premium_core' ) ) ? ' site-logo-text-' . hoot_get_mod( 'logo_size' ) : '';
	...
	$display .= '<div id="' . esc_attr( $id ) . '" class="' . esc_attr( $class ) . '">';
		...
		$display .= '<div class="site-logo-mixed-text">';
			...
			$display .= "<{$tag_h1} " . hoot_get_attr( 'site-title' ) . '>';
				$display .= '<a href="' . esc_url( home_url() ) . '" rel="home" itemprop="url">';
					$title = ( 'mixedcustom' == $unos_logo ) ? unos_get_custom_text_logo() : '<span class="blogname">' . get_bloginfo( 'name' ) . '</span>';
					$display .= apply_filters( 'unos_site_title', $title );
				$display .= "</a>";
			$display .= "</{$tag_h1}>";
...
function unos_get_custom_text_logo() {
	...
	if ( is_array( $logo_custom ) && !empty( $logo_custom ) ) {
		...
		if ( is_customize_preview() ) {
			...
			echo wp_kses( apply_filters( 'unos_customtext_selectiverefresh_inlinecss', '<style type="text/css">' . $css . '</style>' ), hoot_data( 'hootallowedtags' ) );
		}
...
function unos_secondary_menu( $context ) {
	$location = hoot_get_mod( 'menu_location' );
	$side = hoot_get_mod( 'logo_side' );
	if ( $location == 'side' ) { $location = 'none'; $side = 'menu'; }
 ...
 * @return void
 */
if ( !function_exists( 'unos_frontpagearea_fontstyle_customizer' ) ):
function unos_frontpagearea_fontstyle_customizer() {
...
if ( is_customize_preview() ) add_action( 'wp_footer', 'unos_frontpagearea_fontstyle_customizer', 5 );
 ...
 * @return void
 */
if ( !function_exists( 'unos_ns_filter_avatar' ) ):
function unos_ns_filter_avatar( $avatar, $id_or_email, $size, $default, $alt, $args ) {
 ...
 * @return void
 */
if ( !function_exists( 'unos_post_prev_next_links' ) ):
function unos_post_prev_next_links() {
	...
	?><div id="loop-nav-wrap" class="loop-nav"<?php echo $style; ?>><?php
		previous_post_link( '<div class="prev">' . __( 'Previous Post: %link', 'unos' ) . '</div>', '%title' );
		next_post_link(     '<div class="next">' . __( 'Next Post: %link',     'unos' ) . '</div>', '%title' );
	?></div><!-- .loop-nav --><?php
	endif;
}
endif;
