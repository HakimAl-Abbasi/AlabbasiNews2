...
function unos_add_welcome_notice() {
	...
	<div id="hoot-welcome-msg" class="notice notice-success is-dismissible">
		...
		<p><?php
			/* Translators: 1 is the link start markup, 2 is link markup end */
			printf( esc_html__( '%1$sGet started with Unos%2$s', 'unos' ), '<a class="button-secondary" href="' . esc_url( admin_url( 'themes.php?page=unos-welcome&tab=qstart' ) ) . '">', '</a>' );
			?></p>
	</div>
	<?php
}
...
function unos_dismiss_welcome_notice() {
	...
	update_option( 'unos-dismiss-welcome', 1 );
	wp_die();
}
...
function unos_appearance_subpage() {
	...
	<div class="wrap">
		...
		<h1 class="hoot-about-title"><?php
			...
			printf( esc_html__( 'About %1$s %2$s', 'unos' ), hoot_data( 'template_name' ), hoot_data( 'template_version' ) );
			?></h1>

		<div id="hoot-about-sub" class="hoot-about-sub">
			<div class="hoot-about-ss"><?php
				if ( file_exists( hoot_data()->template_dir . 'screenshot.jpg' ) ) echo '<img src="' . esc_url( hoot_data()->template_uri . 'screenshot.jpg' ) . '">';
				elseif ( file_exists( hoot_data()->template_dir . 'screenshot.png' ) ) echo '<img src="' . esc_url( hoot_data()->template_uri . 'screenshot.png' ) . '">';
			?></div>
			<div class="hoot-about-text">
				<p class="hoot-about-intro"><?php esc_html_e( 'Unos is a multipurpose highly flexible WordPress theme built on a SEO friendly framework with fast loading speed. It comes with multiple Theme Customizer options, various blog layouts, WooCommerce support etc among many other features.', 'unos' ) ?></p>
				...
				<?php do_action( 'unos_theme_after_about_textlinks', 'unos' ); ?>
				<?php /*
				<div class="hoot-about-notice2">
					<h3><?php esc_html_e( '1 Click Demo Installation', 'unos' ) ?></h3>
					<p><?php
						/* Translators: The %s are placeholders for HTML, so the order can't be changed. *//*
						printf( esc_html__( 'Use the OCDI plugin to install demo content with a single click to make your site look exactly like the %1$sDemo Site%2$s.%3$sNew users often find it easier to edit existing content rather than starting from scratch.', 'unos' ), '<a href="https://demo.wphoot.com/unos/" target="_blank">', '</a>', '<br />' );
					?></p>
					<p class="hoot-about-textlinks">
						<a class="button" href="https://wphoot.com/support/unos/#docs-section-demo-content-free" target="_blank"><span class="dashicons dashicons-art"></span> <?php esc_html_e( 'Install Demo Content', 'unos' ) ?></a>
					</p>
				</div>
				*/ ?>
			</div>
		...
		<div class="hoot-abouttabs">
			...
			<h2 id="nav-tabs" class="nav-tab-wrapper">
				...
				<?php do_action( 'unos_theme_after_nav_tab_upsell', 'unos', $activetab ); ?>
			...
			<?php do_action( 'unos_theme_after_hoot_upsell', 'unos', $activetab ); ?>
	...
	$quickstart[] = array(
		...
		'desc' => sprintf( esc_html__( '%14$sIt is recommended to install demo content only on a fresh new site%15$s
			...
			%2$s', 'unos' ), '<ol>', '</ol>', '<li>', '</li>', '<strong>', '</strong>', '<em>', '</em>', '<hr>',
										...
										'<a href="https://wphoot.com/support/unos/#docs-section-demo-content-free" target="_blank">',
