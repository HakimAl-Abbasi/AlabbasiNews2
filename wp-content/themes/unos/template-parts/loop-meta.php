 ...
 */
$display_loop_meta = apply_filters( 'unos_display_loop_meta', true );
do_action( 'unos_loop_meta', 'start' );
...
if ( !is_front_page() && !is_singular() ) :
	...
	$display_title = apply_filters( 'unos_loop_meta_display_title', true, 'plural' );
				...
				<div <?php hoot_attr( 'loop-meta', 'archive', 'hgrid-span-12' ); ?>>
					...
					<?php if ( is_author() ) : ?>
						<div class="loop-meta-gravatar"><?php
							...
							$gwidth = apply_filters( 'unos_loop_meta_gravatar', 0 );
							$gwidth = intval( $gwidth );
							$gwidth = ( !empty( $gwidth ) ) ? $gwidth : 150;
							add_filter( 'get_avatar', 'unos_ns_filter_avatar', 10, 6 );
							echo get_avatar( $author->ID, $gwidth, '404' );
							remove_filter( 'get_avatar', 'unos_ns_filter_avatar', 10, 6 );
							?></div>
					<?php endif; ?>
...
elseif ( is_singular() ) :
	...
	if ( have_posts() ) :
		...
		while ( have_posts() ) : the_post();
			...
			$display_title = apply_filters( 'unos_loop_meta_display_title', '', 'singular' );
			if ( $display_title !== 'hide' ) :
					...
					<div class="hgrid">
						...
						<div <?php hoot_attr( 'loop-meta', '', 'hgrid-span-12' ); ?>>
							<div class="entry-header">
								...
								$pretitle = apply_filters( 'unos_singular_loop_pretitle', $pretitle );
								?>
								<h1 <?php hoot_attr( 'loop-title' ); ?>><?php the_title( $pretitle ); ?></h1>

								<?php
								$hide_meta_info = apply_filters( 'unos_hide_meta', false, 'top' );
